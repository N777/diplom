...
import pandas
import requests
from twill.commands import go, formvalue, submit, code


class TimetableParseService:

    ULSTU_TIMETABLE_URL = fr'https://lk.ulstu.ru/timetable/shared/schedule/%D0%A7%D0%B0%D1%81%D1%82%D1%8C%202%20%E2%80%93%20%D0%A4%D0%98%D0%A1%D0%A2,%20%D0%A1%D0%A4,%20%D0%93%D0%A4'

    def login(self):
        go("https://lk.ulstu.ru/?q=auth/login")
        formvalue("1", "login", "v.egorov")
        formvalue("1", "password", "3c817f04")
        submit('0')
        code("200")
        x = requests.get(self.ULSTU_TIMETABLE_URL)
        pass
    ...
    def parse_timetable_from_html(self):
        ...
        for counter in range(1, 100):
            print(counter)
            url = self.ULSTU_TIMETABLE_URL + f"/{counter}.html"
...
TimetableParseService().login()
TimetableParseService().parse_timetable_from_html()
